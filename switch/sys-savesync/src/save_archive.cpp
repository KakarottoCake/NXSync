...
bool dump_save(
    ...
    if (R_FAILED(accountGetLastOpenedUser(&uid))) {
        error = "cannot determine the last active Switch user";
        return false;
    }
    if (R_FAILED(fsdevMountSaveDataReadOnly("save", title_id, uid))) {
        error = "cannot mount account save read-only";
        return false;
    }
...
bool restore_save(
    ...
    if (R_FAILED(fsdevMountSaveData("save", title_id, uid))) {
        error = "cannot mount account save";
        return false;
    }
